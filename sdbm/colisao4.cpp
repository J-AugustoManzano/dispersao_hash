// ============================================================================
// PROGRAMA   : colisao4.cpp
// DESCRIÇÃO  : Script de análise empírica e validação estatística.
//              Mapeia o comportamento da função de hash SDBM,
//              demonstrando visualmente a eficácia da dispersão não linear 
//              e a redução drástica de colisões.
...
// A função de hash SDBM adaptada para a estrutura do programa original
static std::string myHash(std::string mensagem) {
  unsigned long hashCalculado = 0;

  for (char c : mensagem) {
    // Implementação da fórmula SDBM: hash = c + (hash << 6) + (hash << 16) - hash;
    hashCalculado = static_cast<unsigned long>(c) + (hashCalculado << 6) + (hashCalculado << 16) - hashCalculado;
  }
...
int main(void) {
  ...
  // Gerador simples de strings para teste (combinações de 4 caracteres)
  ...
  cout << "==================================================" << endl;
  cout << "RESULTADO DO TESTE DE COLISAO (SDBM) - VERSAO 4.0" << endl;
  cout << "==================================================" << endl;
